...
//    this->resize(1000,520);
    ...
    connect(m_deckUnit, SIGNAL(signal_recv_Delaytime(const int, const double, const double, const double)),
            this, SLOT(slotRecvDelaytime(const int&, const double&, const double&, const double&)));

    connect(m_deckUnit, SIGNAL(signal_Sondary_recv_range(const float, const double, const double, const double)),
            this, SLOT(slotRecvSondaryrange(const float&, const double&, const double&, const double&)));

    connect(m_deckUnit, SIGNAL(signal_recv_sendsignal()),this, SLOT(slotRecvSendSignal()));
    connect(m_deckUnit, SIGNAL(signal_recv_clicked_start()), this, SLOT(slotRecvClickedStart()));
    connect(m_deckUnit, SIGNAL(signal_recv_coordsystem(const double&, const double&, const double&)),
            this, SLOT(slot_recv_coordsystem(const double&, const double&, const double&)));
    connect(m_deckUnit, SIGNAL(signal_recv_coordAfterCalculator(const double&, const double&, const double&)),
            this, SLOT(slot_recv_coordAfterCalculator(const double&, const double&, const double&)));
    connect(m_deckUnit, SIGNAL(signal_send_OBS_info(const QString&, const QString&)),
            this, SLOT(slot_OBS_info(const QString&, const QString&)));
    connect(m_deckUnit, SIGNAL(signal_stop_information()), this, SLOT(slot_stop_deckUnit()));
    testData();
    irecindex=0;
    sondaryflag=-1;
}
...
void GPositionSystem::MedianFilter(QVector<QVector<GobsCoord> >& data, double& pX, double& pY, double& pZ)
{
    ...
    for(int iList1 = 0; iList1 < data.count(); iList1 ++)
    {
        ...
        {
            dataNum += 1;
            XMinNum = min(XMinNum, data[iList1][iList2].x);
            YMinNum = min(YMinNum, data[iList1][iList2].y);
            ZMinNum = min(XMinNum, data[iList1][iList2].z);

            XMaxNum = max(XMaxNum, data[iList1][iList2].x);
            YMaxNum = max(YMaxNum, data[iList1][iList2].y);
            ZMaxNum = max(ZMaxNum, data[iList1][iList2].z);
...
//                -13900,-24658,214,
//                5000,-23394,74};
//    double B[3]={-94713552950.7,-1084567413292.3,-951757426122.44};
    dgesv_(&n,&nrhs,A,&lda,ipiv,B,&ldb,&info);
