...
{
    qDebug()<< __LINE__ <<"\t Preprocess";//log
    //1 读取sps文件-------------
...
bool RecvOrientedSegy::resvShotInform()
{
    ...
    {
        qDebug() << "dataFile = " << m_depolyedDevices[i]->dataFile;
        if(!m_depolyedDevices[i]->dataFile.isEmpty())
        {
            ...
            QStringList logFiles = dir.entryList(nameFilters,QDir::NoDotAndDotDot|QDir::Files,QDir::Name);
...
QList<DepolyedDevice *> RecvOrientedSegy::checkEffectiveDevice()
{
    ...
    for(int i =0;i<m_depolyedDevices.size();i++)//m_depolyedDevices.size()
    {
        ...
        {
            qDebug() << "startLine = " << startLine << "\t lastLine = " << lastLine
                     << "\t lineNo = " << lineNo
                     << "\t startStation = " << startStation << "\t lastStation = " << lastStation
                     << "\t staNo = " << staNo;
            //判断范围
...
bool RecvOrientedSegy::readLogFile(DataFileInfo *dataFileInfo)
{
    ...
    QString logFile = dataFileInfo->dataPath + QString("/A%1.log").arg(startDateStr);
    QFile file(logFile);
    if(!file.exists())
    {
...
bool RecvOrientedSegy::getDataFiles(const QString &dataPath,DataFileInfo *dataFileInfo,QString *errStr)
{
    ...
    //起始时间,正确的值，校正基准值，计算以后每个文件的时间
    QStringList strList = fileList[idx0].split(QRegExp("\\s+"));
    QDateTime fileTime  = QDateTime::fromString(QString("%1 %2").arg(strList[1],strList[2].left(12)),
                                                DateTime_Format_);
    //采样率
    dataFileInfo->frequency = strList[4].remove("sps").toInt();
    dataFileInfo->startTime = fileTime;
...
// qDebug() << "startDateTime = " << startDateTime << "\t finalDateTime = " << finalDateTime << "\t" << shotLineTime.shotTimes[shot];
