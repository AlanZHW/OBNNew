...
{
    this->setWindowTitle(QString("DMC(Main Window)|Project:%1").arg(m_projName));
    ...
    //===
    m_TimeError->setProjInfo(&m_projInfo);
    return true;
}
...
void DataMgrControl::initWindow()
{
    ...
    //数据管理页
    //====时差
    m_TimeError = new GobsDtMagerDiffTime;
    QVBoxLayout *pTimeErrorLayout = new QVBoxLayout;
    pTimeErrorLayout->addWidget(m_TimeError);
    ui->tabWidget->widget(0)->setLayout(pTimeErrorLayout);
    ui->TimeError->setLayout(pTimeErrorLayout);
...
void DataMgrControl::slotNewSegyData()
{
    if(!m_ExportSegyDlg)
    {
        ...
        connect(m_ExportSegyDlg,SIGNAL(signalStartExport(ExportSegyParameter)),this,SLOT(startExportSegyData(ExportSegyParameter)));
